...
void mg_resolve_cancel(struct mg_connection *c) {
  struct dns_data *tmp, *d = c->mgr ? (struct dns_data *) c->mgr->active_dns_requests : NULL;
  for (; d != NULL; d = tmp) {
    tmp = d->next;
...
//Access-Control-Max-Age Ô¤¼ìÇëÇó
//Access-Control-Allow-Origin ·Å¿í
...
void mg_http_reply_scz(struct mg_connection* c, int code, const char* headers,
    const char* body, size_t body_len) {
	mg_printf(c, "HTTP/1.1 %d %s\r\n%sContent-Length: %d\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Headers: *\r\nAccess-Control-Allow-Methods: POST, GET, OPTIONS\r\nConnection: keep-alive\r\nAccess-Control-Max-Age: 3600\r\nContent-Type: application/json\r\nKeep-Alive: timeout=75\r\n\r\n", code,
		mg_http_status_code_str(code), headers == NULL ? "" : headers, body_len);
